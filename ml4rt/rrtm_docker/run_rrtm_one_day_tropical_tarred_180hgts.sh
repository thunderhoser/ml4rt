...
shopt -s nullglob
tarred_rap_file_names=${TOP_DATA_DIR_NAME_ACTUAL}/profiler_sites*${date_string}.tar.gz
tarred_rap_file_name=${tarred_rap_file_names[0]}

echo "Found RAP file for date ${date_string}: ${tarred_rap_file_name}"
...
tar -C $data_dir_name_actual -xvf $tarred_rap_file_name
echo $SEPARATOR_STRING

shopt -s nullglob
actual_rap_file_names=${data_dir_name_actual}/profiler_sites*${date_string}*00.nc
