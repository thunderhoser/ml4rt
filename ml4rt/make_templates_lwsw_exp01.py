"""Makes templates for first experiment with longwave and shortwave."""
...
OUTPUT_DIR_NAME = (
    '{0:s}/ml4rt_models/lwsw_experiment01/templates'
...
DENSE_LAYER_NEURON_COUNTS = architecture_utils.get_dense_layer_dimensions(
    num_input_units=7 * 1024, num_classes=4, num_dense_layers=4,
...
def _run():
    ...
    for i in range(num_scaling_factors):
        this_scalar_loss_function = custom_losses.scaled_mse_for_net_fluxes(
            ...
            down_flux_indices=numpy.array([0, 2], dtype=int),
            up_flux_indices=numpy.array([1, 3], dtype=int)
        )
        this_scalar_loss_string = (
            'custom_losses.scaled_mse_for_net_fluxes(scaling_factor={0:.10f}, '
            'down_flux_indices=numpy.array([0, 2], dtype=int), '
            'up_flux_indices=numpy.array([1, 3], dtype=int))'.format(
