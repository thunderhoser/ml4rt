"""Makes U-net++ templates for Exp 2 with spectrally resolved shortwave RT.

Same as actual Experiment 2 but experimenting with batch sizes.
"""
...
OUTPUT_DIR_NAME = (
    '{0:s}/ml4rt_models/spectral_experiment02_batch_size/shortwave_plusplus/templates'
    ...
    for i in range(num_depths):
        for j in range(num_conv_layer_counts):
            for k in range(num_channel_counts):
                for m in range(len(GRAD_ACCUM_STEP_COUNTS)):
                    ...
                    if GRAD_ACCUM_STEP_COUNTS[m] == 1:
                        this_optimizer_function = keras.optimizers.Nadam()
                        this_optimizer_string = 'keras.optimizers.Nadam()'
                    else:
                        this_optimizer_function = keras.optimizers.Nadam(
                            gradient_accumulation_steps=GRAD_ACCUM_STEP_COUNTS[m]
                        )
                        this_optimizer_string = (
                            'keras.optimizers.Nadam(gradient_accumulation_steps={0:d})'
                        ).format(GRAD_ACCUM_STEP_COUNTS[m])

                    this_option_dict[u_net_pp_arch.OPTIMIZER_FUNCTION_KEY] = (
                        this_optimizer_function
                    )

                    this_model_object = u_net_pp_arch.create_model(this_option_dict)

                    this_model_file_name = (
                        '{0:s}/num-grad-accum-steps={1:02d}/model.keras'
                    ).format(
                        OUTPUT_DIR_NAME,
                        GRAD_ACCUM_STEP_COUNTS[m]
                    )
