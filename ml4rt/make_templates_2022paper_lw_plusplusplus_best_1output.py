"""Makes template for best longwave model in 2022 paper."""
...
OUTPUT_DIR_NAME = (
    '{0:s}/ml4rt_models/2022paper_experiment_lw_plusplusplus_best_1output/'
    'template'
).format(HOME_DIR_NAME)

LOSS_FUNCTION = custom_losses.joined_output_loss(127, 1.)
LOSS_FUNCTION_STRING = 'custom_losses.joined_output_loss(127, 1.)'

MODEL_DEPTHS = numpy.array([5], dtype=int)
CONV_LAYER_COUNTS = numpy.array([1], dtype=int)
FIRST_LAYER_CHANNEL_COUNTS = numpy.array([64], dtype=int)
...
DEFAULT_OPTION_DICT = {
    ...
    u_net_ppp_architecture.INCLUDE_PENULTIMATE_KEY: False,
        ...
        for j in range(num_conv_layer_counts):
            for k in range(num_channel_counts):
                ...
                this_option_dict.update({
                    u_net_ppp_architecture.NUM_LEVELS_KEY:
                        MODEL_DEPTHS[i],
                    u_net_ppp_architecture.CONV_LAYER_COUNTS_KEY:
                        numpy.full(
                            MODEL_DEPTHS[i] + 1, CONV_LAYER_COUNTS[j], dtype=int
                        ),
                    u_net_ppp_architecture.CHANNEL_COUNTS_KEY:
                        these_channel_counts_all,
                    u_net_ppp_architecture.ENCODER_DROPOUT_RATES_KEY:
                        numpy.full(MODEL_DEPTHS[i] + 1, 0.),
                    u_net_ppp_architecture.ENCODER_MC_DROPOUT_FLAGS_KEY:
                        numpy.full(MODEL_DEPTHS[i] + 1, 0, dtype=bool),
                    u_net_ppp_architecture.UPCONV_DROPOUT_RATES_KEY:
                        numpy.full(MODEL_DEPTHS[i], 0.),
                    u_net_ppp_architecture.UPCONV_MC_DROPOUT_FLAGS_KEY:
                        numpy.full(MODEL_DEPTHS[i], 0, dtype=bool),
                    u_net_ppp_architecture.SKIP_DROPOUT_RATES_KEY:
                        numpy.full(MODEL_DEPTHS[i], 0.),
                    u_net_ppp_architecture.SKIP_MC_DROPOUT_FLAGS_KEY:
                        numpy.full(MODEL_DEPTHS[i], 0, dtype=bool)
                })
                ...
                this_option_dict[
                    u_net_ppp_architecture.DENSE_LAYER_NEURON_NUMS_KEY
                ] = these_neuron_counts
