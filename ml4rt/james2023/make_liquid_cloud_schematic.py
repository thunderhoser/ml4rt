"""Makes schematic to show how liquid-cloud layers are created."""
...
#     0.693646, 0.347054
# ])
ORIG_WATER_CONTENTS_KG_M03[55:60] = 0.001 * numpy.array([
    0.003691, 0.035799, 0.111804, 0.460531, 0.347054
])

WHOLE_PROFILE_TEMP_OFFSET_KELVINS = 2.
...
def _run(output_dir_name):
    ...
    panel_file_names = [
        '{0:s}/liquid_cloud_part1.jpg'.format(output_dir_name)
    ]
    ...
    for k in range(1, num_cloud_layers):
        ...
        good_temperature_flags = (
            ...
            >= perturb_gfs.MIN_TEMP_FOR_LIQUID_CLOUD_KELVINS
        )
    ...
    panel_file_names.append(
        '{0:s}/liquid_cloud_part2.jpg'.format(output_dir_name)
    )
    ...
    axes_objects[0].set_title(
        ...
        r'(temperature $\geq$ -40 $^{\circ}$C and no overlap)'
    )
    gg_plotting_utils.label_axes(
    ...
    panel_file_names.append(
        '{0:s}/liquid_cloud_part3.jpg'.format(output_dir_name)
    )
    ...
    axes_object.set_xlabel(r'Liquid-water content (g m$^{-3}$)')
    axes_object.set_ylabel('Height (km AGL)')
    axes_object.set_title('LWC profile with new cloud')
    gg_plotting_utils.label_axes(axes_object=axes_object, label_string='(d)')

    panel_file_names.append(
        '{0:s}/liquid_cloud_part4.jpg'.format(output_dir_name)
    )
    ...
    axes_object.set_title(
        'LWC profile with new cloud\n(including Gaussian noise)'
    )
    gg_plotting_utils.label_axes(axes_object=axes_object, label_string='(e)')

    panel_file_names.append(
        '{0:s}/liquid_cloud_part5.jpg'.format(output_dir_name)
    )
    ...
    panel_file_names.append(
        '{0:s}/liquid_cloud_part6.jpg'.format(output_dir_name)
    )
    ...
    concat_figure_file_name = '{0:s}/liquid_cloud_schematic.jpg'.format(
        output_dir_name
    )
