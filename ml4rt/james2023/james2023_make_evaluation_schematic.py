...
def _plot_attr_diag_poor_model(output_dir_name, panel_letter):
    ...
    title_string = 'Attributes diagram for Model B\nREL = {0:.2f}'.format(
        reliability_k2_day02
    )
...
def _plot_attr_diag_good_model(output_dir_name, panel_letter):
    ...
    title_string = 'Attributes diagram for Model A\nREL = {0:.2f}'.format(
        reliability_k2_day02
    )
    title_string += r' K$^2$ day$^{-2}$'
    axes_object.set_title(title_string)
...
def _plot_spread_vs_skill_poor_model(output_dir_name, panel_letter):
    ...
    these_dim_keys_no_bins = (ss_utils.VECTOR_FIELD_DIM,)
    these_dim_keys_no_edge = (
        ss_utils.VECTOR_FIELD_DIM, ss_utils.HEATING_RATE_BIN_DIM
    )
    these_dim_keys_with_edge = (
        ss_utils.VECTOR_FIELD_DIM, ss_utils.HEATING_RATE_BIN_EDGE_DIM
    )
    d = result_dict
    ...
    main_data_dict = {
        ss_utils.VECTOR_FLAT_MEAN_STDEV_KEY: (
            these_dim_keys_no_edge,
            numpy.expand_dims(d[ss_utils.MEAN_PREDICTION_STDEVS_KEY], axis=0)
        ),
        ss_utils.VECTOR_FLAT_BIN_EDGE_KEY: (
            these_dim_keys_with_edge,
            numpy.expand_dims(
                d[ss_utils.BIN_EDGE_PREDICTION_STDEVS_KEY], axis=0
            )
        ),
        ss_utils.VECTOR_FLAT_RMSE_KEY: (
            these_dim_keys_no_edge,
            numpy.expand_dims(d[ss_utils.RMSE_VALUES_KEY], axis=0)
        ),
        ss_utils.VECTOR_FLAT_SSREL_KEY: (
            these_dim_keys_no_bins,
            numpy.array([d[ss_utils.SPREAD_SKILL_RELIABILITY_KEY]])
        ),
        ss_utils.VECTOR_FLAT_SSRAT_KEY: (
            these_dim_keys_no_bins,
            numpy.array([d[ss_utils.SPREAD_SKILL_RATIO_KEY]])
        ),
        ss_utils.VECTOR_FLAT_EXAMPLE_COUNT_KEY: (
            these_dim_keys_no_edge,
            numpy.expand_dims(d[ss_utils.EXAMPLE_COUNTS_KEY], axis=0)
        ),
        ss_utils.VECTOR_FLAT_MEAN_MEAN_PREDICTION_KEY: (
            these_dim_keys_no_edge,
            numpy.expand_dims(d[ss_utils.MEAN_MEAN_PREDICTIONS_KEY], axis=0)
        ),
        ss_utils.VECTOR_FLAT_MEAN_TARGET_KEY: (
            these_dim_keys_no_edge,
            numpy.expand_dims(d[ss_utils.MEAN_TARGET_VALUES_KEY], axis=0)
        )
    }
    ...
    title_string = 'Spread vs. skill for Model B\nSSREL = {0:.3f}'.format(
        result_dict[ss_utils.SPREAD_SKILL_RELIABILITY_KEY]
    )
    title_string += r' K day$^{-1}$'
    title_string += '; SSRAT = {0:.3f}'.format(
        result_dict[ss_utils.SPREAD_SKILL_RATIO_KEY]
    )
    axes_object.set_title(title_string)
...
def _plot_spread_vs_skill_good_model(output_dir_name, panel_letter):
    ...
    title_string = 'Spread vs. skill for Model A\nSSREL = {0:.3f}'.format(
        result_dict[ss_utils.SPREAD_SKILL_RELIABILITY_KEY]
    )
    title_string += r' K day$^{-1}$'
    title_string += '; SSRAT = {0:.3f}'.format(
...
def _plot_discard_test_poor_model(output_dir_name, panel_letter):
    ...
    these_dim_keys_1d = (dt_utils.VECTOR_FIELD_DIM,)
    these_dim_keys_2d = (
        dt_utils.VECTOR_FIELD_DIM, dt_utils.UNCERTAINTY_THRESHOLD_DIM
    )
    these_dim_1d = 1
    these_dim_2d = (1, num_thresholds)
    ...
    use_example_flags = numpy.full(SAMPLE_SIZE, True, dtype=bool)
    ...
    for i in range(len(uncertainty_thresholds)):
        ...
        t[dt_utils.EXAMPLE_FRACTION_KEY].values[i] = numpy.mean(
            use_example_flags
        )
        t[dt_utils.POST_DISCARD_ERROR_KEY].values[i] = error_function(
            actual_heating_rates_k_day01, predicted_hr_matrix_k_day01,
            use_example_flags
        )
        ...
        t[dt_utils.VECTOR_FLAT_MEAN_MEAN_PREDICTION_KEY].values[0, i] = (
            numpy.mean(this_mean_pred_by_example)
        )
        t[dt_utils.VECTOR_FLAT_MEAN_TARGET_KEY].values[0, i] = numpy.mean(
            actual_heating_rates_k_day01[use_example_flags]
        )
        t[dt_utils.VECTOR_FLAT_POST_DISCARD_ERROR_KEY].values[0, i] = (
            error_function(
                actual_heating_rates_k_day01, predicted_hr_matrix_k_day01,
                use_example_flags
            )
    ...
    discard_fractions = 1. - t[dt_utils.EXAMPLE_FRACTION_KEY].values
    t[dt_utils.VECTOR_FLAT_MONO_FRACTION_KEY].values[0] = numpy.mean(
        numpy.diff(t[dt_utils.VECTOR_FLAT_POST_DISCARD_ERROR_KEY].values[0, :])
        < 0
    )
    t[dt_utils.VECTOR_FLAT_MEAN_DI_KEY].values[0] = numpy.mean(
        -1 *
        numpy.diff(t[dt_utils.VECTOR_FLAT_POST_DISCARD_ERROR_KEY].values[0, :])
    ...
    title_string = 'Discard test for Model B\nMF = {0:.1f}%'.format(
        100 * t[dt_utils.VECTOR_FLAT_MONO_FRACTION_KEY].values[0]
    )
    axes_object.set_title(title_string)
...
def _plot_pit_histogram_good_model(output_dir_name, panel_letter):
    """Plots PIT histogram for good model.
    ...
    these_dim_keys_1d = (pit_utils.VECTOR_FIELD_DIM,)
    these_dim_keys_2d = (pit_utils.VECTOR_FIELD_DIM, pit_utils.BIN_CENTER_DIM)

    main_data_dict = {
        pit_utils.VECTOR_FLAT_PITD_KEY: (
            these_dim_keys_1d,
            numpy.array([result_dict[pit_utils.PITD_KEY]])
        ),
        pit_utils.VECTOR_FLAT_PERFECT_PITD_KEY: (
            these_dim_keys_1d,
            numpy.array([result_dict[pit_utils.PERFECT_PITD_KEY]])
        ),
        pit_utils.VECTOR_FLAT_BIN_COUNT_KEY: (
            these_dim_keys_2d,
            numpy.expand_dims(result_dict[pit_utils.BIN_COUNTS_KEY], axis=0)
        ),
        pit_utils.VECTOR_FLAT_LOW_BIN_BIAS_KEY: (
            these_dim_keys_1d,
            numpy.array([result_dict[pit_utils.LOW_BIN_BIAS_KEY]])
        ),
        pit_utils.VECTOR_FLAT_MIDDLE_BIN_BIAS_KEY: (
            these_dim_keys_1d,
            numpy.array([result_dict[pit_utils.MIDDLE_BIN_BIAS_KEY]])
        ),
        pit_utils.VECTOR_FLAT_HIGH_BIN_BIAS_KEY: (
            these_dim_keys_1d,
            numpy.array([result_dict[pit_utils.HIGH_BIN_BIAS_KEY]])
        ),
        pit_utils.VECTOR_FLAT_EXTREME_PIT_FREQ_KEY: (
            these_dim_keys_1d,
            numpy.array([result_dict[pit_utils.EXTREME_PIT_FREQ_KEY]])
        )
    }
    ...
    title_string = 'PIT histogram for Model A\nPITD = {0:.4f}'.format(
        result_dict[pit_utils.PITD_KEY]
    )
    axes_object.set_title(title_string)

    output_file_name = '{0:s}/pit_histogram_good_model.jpg'.format(
        output_dir_name
    )
    print('Saving figure to: "{0:s}"...'.format(output_file_name))
    figure_object.savefig(
...
def _plot_pit_histogram_poor_model(output_dir_name, panel_letter):
    """Plots PIT histogram for poor model.
    ...
    title_string = 'PIT histogram for Model B\nPITD = {0:.4f}'.format(
        result_dict[pit_utils.PITD_KEY]
    )
    axes_object.set_title(title_string)

    output_file_name = '{0:s}/pit_histogram_poor_model.jpg'.format(
        output_dir_name
    )
    print('Saving figure to: "{0:s}"...'.format(output_file_name))
    figure_object.savefig(
...
def _run(output_dir_name):
    ...
    panel_file_names.append(_plot_pit_histogram_good_model(
        output_dir_name=output_dir_name, panel_letter='i'
    ))
    panel_file_names.append(_plot_pit_histogram_poor_model(
        output_dir_name=output_dir_name, panel_letter='j'
    ))
