"""Plots scores on hyperparameter grid for GFS experiment with batch size."""
...
EXAMPLES_PER_BATCH_COUNTS = numpy.round(
    numpy.logspace(6, 9, num=7, base=2)
).astype(int)

BATCHES_PER_EPOCH_COUNTS = numpy.array([100, 250, 500, 750, 1000], dtype=int)
...
def _print_ranking_one_score(score_matrix, score_name):
    ...
    E = number of examples-per-batch counts
    B = number of batches-per-epoch counts

    :param score_matrix: E-by-B numpy array of scores.
    ...
    for m in range(len(i_sort_indices)):
        ...
        print((
            '{0:d}th-lowest {1:s} = {2:.4g} ... examples per batch = {3:d} ... '
            'batches per epoch = {4:d}'
        ).format(
            m + 1, score_name, score_matrix[i, j], EXAMPLES_PER_BATCH_COUNTS[i],
            BATCHES_PER_EPOCH_COUNTS[j]
        ))
...
def _print_ranking_all_scores(
    ...
    :param prmse_matrix_k_day01: E-by-B numpy array of profile RMSE.
    :param dwmse_matrix_k3_day03: E-by-B numpy array of dual-weighted MSE.
    :param down_flux_rmse_matrix_w_m02: E-by-B numpy array of RMSE for
        surface downwelling flux.
    :param up_flux_rmse_matrix_w_m02: E-by-B numpy array of RMSE for TOA
        upwelling flux.
    :param net_flux_rmse_matrix_w_m02: E-by-B numpy array of RMSE for net
        flux.
    """
    ...
    for m in range(len(i_sort_indices)):
        ...
        print((
            ...
            'examples per batch = {2:d} ... batches per epoch = {3:d} ... '
            'DWMSE rank = {4:.1f} ... down-flux-RMSE rank = {5:.1f} ... '
            'up-flux-RMSE rank = {6:.1f} ... net-flux-RMSE rank = {7:.1f}'
        ).format(
            m + 1, prmse_matrix_k_day01[i, j],
            EXAMPLES_PER_BATCH_COUNTS[i], BATCHES_PER_EPOCH_COUNTS[j],
            dwmse_rank_matrix[i, j], down_flux_rmse_rank_matrix[i, j],
            up_flux_rmse_rank_matrix[i, j],
            net_flux_rmse_rank_matrix[i, j]
        ))
...
def _run(experiment_dir_name, isotonic_flag):
    ...
    num_example_counts = len(EXAMPLES_PER_BATCH_COUNTS)
    num_batch_counts = len(BATCHES_PER_EPOCH_COUNTS)

    y_tick_labels = ['{0:d}'.format(n) for n in EXAMPLES_PER_BATCH_COUNTS]
    x_tick_labels = ['{0:d}'.format(n) for n in BATCHES_PER_EPOCH_COUNTS]

    y_axis_label = 'Examples per batch'
    x_axis_label = 'Batches per epoch'
    ...
    if os.path.isfile(score_file_name):
        ...
        dimensions = (num_example_counts, num_batch_counts)
        ...
        for i in range(num_example_counts):
            for j in range(num_batch_counts):
                this_model_dir_name = (
                    '{0:s}/num-examples-per-batch={1:03d}_'
                    'num-batches-per-epoch={2:04d}'
                ).format(
                    experiment_dir_name, EXAMPLES_PER_BATCH_COUNTS[i],
                    BATCHES_PER_EPOCH_COUNTS[j]
                )
                ...
                (
                    ...
                    down_flux_rmse_matrix_w_m02[i, j],
                    up_flux_rmse_matrix_w_m02[i, j],
    ...
    figure_object, axes_object = _plot_scores_2d(
        ...
        max_colour_value=numpy.nanpercentile(prmse_matrix_k_day01, 100),
        x_tick_labels=x_tick_labels, y_tick_labels=y_tick_labels
    )

    figure_width_px = figure_object.get_size_inches()[0] * figure_object.dpi
    marker_size_px = figure_width_px * (
        BEST_MARKER_SIZE_GRID_CELLS / prmse_matrix_k_day01.shape[1]
    )
    axes_object.plot(
        min_prmse_indices[0], min_prmse_indices[1],
    ...
    axes_object.set_title(r'PRMSE (K day$^{-1}$)')

    figure_file_name = '{0:s}/{1:s}prmse_grid.jpg'.format(
        experiment_dir_name,
        'isotonic_regression/' if isotonic_flag else ''
    )
    ...
    figure_object, axes_object = _plot_scores_2d(
        ...
        max_colour_value=numpy.nanpercentile(dwmse_matrix_k3_day03, 100),
        x_tick_labels=x_tick_labels, y_tick_labels=y_tick_labels
    )
    ...
    axes_object.plot(
        min_dwmse_indices[0], min_dwmse_indices[1],
        linestyle='None', marker=BEST_MARKER_TYPE,
        markersize=marker_size_px, markeredgewidth=0,
        markerfacecolor=MARKER_COLOUR,
        markeredgecolor=MARKER_COLOUR
    ...
    axes_object.set_xlabel(x_axis_label)
    axes_object.set_ylabel(y_axis_label)
    axes_object.set_title(r'DWMSE (K$^{3}$ day$^{-3}$)')

    figure_file_name = '{0:s}/{1:s}dwmse_grid.jpg'.format(
        experiment_dir_name,
        'isotonic_regression/' if isotonic_flag else ''
    )
    file_system_utils.mkdir_recursive_if_necessary(
    ...
    figure_object, axes_object = _plot_scores_2d(
        score_matrix=down_flux_rmse_matrix_w_m02,
        min_colour_value=numpy.nanpercentile(down_flux_rmse_matrix_w_m02, 0),
        max_colour_value=numpy.nanpercentile(down_flux_rmse_matrix_w_m02, 100),
        x_tick_labels=x_tick_labels, y_tick_labels=y_tick_labels
    )

    figure_width_px = figure_object.get_size_inches()[0] * figure_object.dpi
    marker_size_px = figure_width_px * (
        BEST_MARKER_SIZE_GRID_CELLS / down_flux_rmse_matrix_w_m02.shape[1]
    )
    axes_object.plot(
        min_down_flux_rmse_indices[0], min_down_flux_rmse_indices[1],
        linestyle='None', marker=BEST_MARKER_TYPE,
        markersize=marker_size_px, markeredgewidth=0,
        markerfacecolor=MARKER_COLOUR,
        markeredgecolor=MARKER_COLOUR
    ...
    axes_object.set_xlabel(x_axis_label)
    axes_object.set_ylabel(y_axis_label)
    axes_object.set_title(r'RMSE for surface downwelling flux (W m$^{-2}$)')

    figure_file_name = '{0:s}/{1:s}down-flux-rmse_grid.jpg'.format(
        experiment_dir_name,
        'isotonic_regression/' if isotonic_flag else ''
    )
    file_system_utils.mkdir_recursive_if_necessary(
    ...
    figure_object, axes_object = _plot_scores_2d(
        ...
        max_colour_value=numpy.nanpercentile(up_flux_rmse_matrix_w_m02, 100),
        x_tick_labels=x_tick_labels, y_tick_labels=y_tick_labels
    )

    figure_width_px = figure_object.get_size_inches()[0] * figure_object.dpi
    marker_size_px = figure_width_px * (
        BEST_MARKER_SIZE_GRID_CELLS / up_flux_rmse_matrix_w_m02.shape[1]
    )
    axes_object.plot(
        min_up_flux_rmse_indices[0], min_up_flux_rmse_indices[1],
        linestyle='None', marker=BEST_MARKER_TYPE,
        markersize=marker_size_px, markeredgewidth=0,
        markerfacecolor=MARKER_COLOUR,
        markeredgecolor=MARKER_COLOUR
    ...
    axes_object.set_xlabel(x_axis_label)
    axes_object.set_ylabel(y_axis_label)
    axes_object.set_title(r'RMSE for TOA upwelling flux (W m$^{-2}$)')

    figure_file_name = '{0:s}/{1:s}up-flux-rmse_grid.jpg'.format(
        experiment_dir_name,
        'isotonic_regression/' if isotonic_flag else ''
    )
    file_system_utils.mkdir_recursive_if_necessary(
    ...
    figure_object, axes_object = _plot_scores_2d(
        ...
        max_colour_value=numpy.nanpercentile(net_flux_rmse_matrix_w_m02, 100),
        x_tick_labels=x_tick_labels, y_tick_labels=y_tick_labels
    )

    figure_width_px = figure_object.get_size_inches()[0] * figure_object.dpi
    marker_size_px = figure_width_px * (
        BEST_MARKER_SIZE_GRID_CELLS / net_flux_rmse_matrix_w_m02.shape[1]
    )
    axes_object.plot(
        min_net_flux_rmse_indices[0], min_net_flux_rmse_indices[1],
        linestyle='None', marker=BEST_MARKER_TYPE,
        markersize=marker_size_px, markeredgewidth=0,
        markerfacecolor=MARKER_COLOUR,
        markeredgecolor=MARKER_COLOUR
    ...
    axes_object.set_xlabel(x_axis_label)
    axes_object.set_ylabel(y_axis_label)
    axes_object.set_title(r'RMSE for net flux (W m$^{-2}$)')

    figure_file_name = '{0:s}/{1:s}net-flux-rmse_grid.jpg'.format(
        experiment_dir_name,
        'isotonic_regression/' if isotonic_flag else ''
    )
    file_system_utils.mkdir_recursive_if_necessary(
