...
def get_1d_depthwise_conv_layer(
        num_kernel_rows, num_rows_per_stride,
        padding_type_string=NO_PADDING_STRING, weight_regularizer=None,
        layer_name=None, kernel_init_name=KERNEL_INITIALIZER_NAME,
        bias_init_name=BIAS_INITIALIZER_NAME):
    """Creates layer for 1-D depthwise convolution.
    ...
    :return: layer_object: Instance of `keras.layers.DepthwiseConv1D`.
    """
    ...
    _check_convolution_options(
        ...
        num_rows_per_stride=num_rows_per_stride,
        padding_type_string=padding_type_string,
        num_filters=100, num_kernel_dimensions=1
    )

    return keras.layers.DepthwiseConv1D(
        kernel_size=(num_kernel_rows,),
        strides=(num_rows_per_stride,),
...
def get_2d_depthwise_conv_layer(
    ...
    :return: layer_object: Instance of `keras.layers.DepthwiseConv2D`.
    """
