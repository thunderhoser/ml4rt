...
;
;---------------------------------------------------------------------------------
function create_output_file, name, heights_m_agl

  fid = ncdf_create(name,/clobber)
  did0 = ncdf_dimdef(fid,'time',/unlimited)
  did1 = ncdf_dimdef(fid,'height',n_elements(heights_m_agl))
  ...
  vid = ncdf_vardef(fid,'height_m_agl',/float,did1)
  ...
  ncdf_varput,fid,'height_m_agl',heights_m_agl
  ncdf_close,fid
return, 0
end
;---------------------------------------------------------------------------------
function append_to_output_file, name, index, valid_times_unix_sec, julian_days, $
    ...
    temps_kelvins, pressures_mb, vapour_mixing_ratios_g_kg01, $
...
pro runit, year
  ...
  for i=0L,count-1 do begin ; { 
    ...
    ncdf_varget,fid,'height_m_agl',heights_m_agl
    ...
    for j=0,n_elements(sites)-1 do begin ; {
      ...
      for k=0,n_elements(hours)-1 do begin ; {
	    ...
	    if(nbar eq 1) then begin
	      ...
	      endelse
          feh = where(satmos.z(*,stdatmos) gt max(heights_m_agl/1000.+2) and satmos.z(*,stdatmos) le 50,nfeh)
	      if(nfeh gt 0) then begin
                ; Append on the bit of standard atmosphere needed to get high into the stratosphere
	        these_heights_km_agl = [0.001*heights_m_agl,                           satmos.z(feh,stdatmos)]
            ...
            these_heights_km_agl = 0.001*heights_m_agl
    ...
    if(n_elements(output_times_unix_sec) gt 0) then begin
      ...
      if(do_create_output eq 1) then begin
	    index = 0
        do_create_output = create_output_file(outname, 1000*these_heights_km_agl)
      endif
      ...
      index = append_to_output_file(outname, index, output_times_unix_sec, output_julian_days, $
	    ...
	    output_temps_kelvins, output_pressures_mb, output_vapour_mixing_ratios_g_kg01, $
