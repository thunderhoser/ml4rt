...
; for the machine learning (ML) project that will try to approximate the longwave
; RRTM model.
;
; Inputs:
;    The "extracted column" datasets that I have been creating from the RAP
;       From these files, I will use the PTU and cloud profiles as input to the RRTM_SW and RRTM_LW
; Outputs:
;    A netCDF file that contains the input profiles, along with the computed SW and LW fluxes
...
function create_output_file, name, num_heights
  ...
  vid = ncdf_vardef(fid,'aerosol_asymmetry_param',/float,did0)	
      ...
      ncdf_attput,fid,vid,'units','unitless'
  vid = ncdf_vardef(fid,'shortwave_heating_rate_k_day01',/float,[did1,did0])
    ncdf_attput,fid,vid,'long_name','SW radiative heating rate'
    ncdf_attput,fid,vid,'units','K/day'
  vid = ncdf_vardef(fid,'shortwave_downwelling_flux_w_m02',/float,[did1,did0])
    ncdf_attput,fid,vid,'long_name','SW downwelling flux'
    ncdf_attput,fid,vid,'units','W/m2'
  vid = ncdf_vardef(fid,'shortwave_upwelling_flux_w_m02',/float,[did1,did0])
    ncdf_attput,fid,vid,'long_name','SW upwelling flux'
    ncdf_attput,fid,vid,'units','W/m2'
  vid = ncdf_vardef(fid,'shortwave_surface_downwelling_flux_w_m02',/float,did0)
    ncdf_attput,fid,vid,'long_name','SW downwelling flux at the surface'
    ncdf_attput,fid,vid,'units','W/m2'
  vid = ncdf_vardef(fid,'shortwave_toa_upwelling_flux_w_m02',/float,did0)
    ncdf_attput,fid,vid,'long_name','SW upwelling flux at the top of the atmosphere'
    ncdf_attput,fid,vid,'units','W/m2'
  vid = ncdf_vardef(fid,'longwave_heating_rate_k_day01',/float,[did1,did0])
    ncdf_attput,fid,vid,'long_name','LW radiative heating rate'
    ncdf_attput,fid,vid,'units','K/day'
  vid = ncdf_vardef(fid,'longwave_downwelling_flux_w_m02',/float,[did1,did0])
    ncdf_attput,fid,vid,'long_name','LW downwelling flux'
    ncdf_attput,fid,vid,'units','W/m2'
  vid = ncdf_vardef(fid,'longwave_upwelling_flux_w_m02',/float,[did1,did0])
    ncdf_attput,fid,vid,'long_name','LW upwelling flux'
    ncdf_attput,fid,vid,'units','W/m2'
  vid = ncdf_vardef(fid,'longwave_surface_downwelling_flux_w_m02',/float,did0)
    ncdf_attput,fid,vid,'long_name','LW downwelling flux at the surface'
    ncdf_attput,fid,vid,'units','W/m2'
  vid = ncdf_vardef(fid,'longwave_toa_upwelling_flux_w_m02',/float,did0)
  ...
  ncdf_attput,fid,/global,'Comment1','Radiative flux calculations and atmospheric state inputs'
  ncdf_attput,fid,/global,'Comment2','Calculations made with RRTM_SW and RRTM_LW'
...
function append_to_output_file, name, index, valid_times_unix_sec, julian_days, zenith_angles_deg, $
    standard_atmo_enums, latitudes_deg_n, longitudes_deg_e, surface_albedos, surface_temperatures_kelvins, surface_emissivities, $
    ...
    shortwave_heating_rates_k_day01, shortwave_upwelling_fluxes_w_m02, shortwave_downwelling_fluxes_w_m02, shortwave_toa_upwelling_fluxes_w_m02, shortwave_sfc_downwelling_fluxes_w_m02, $
    longwave_heating_rates_k_day01, longwave_upwelling_fluxes_w_m02, longwave_downwelling_fluxes_w_m02, longwave_toa_upwelling_fluxes_w_m02, longwave_sfc_downwelling_fluxes_w_m02
  ...
  ncdf_varput,fid,'shortwave_heating_rate_k_day01',shortwave_heating_rates_k_day01,offset=[0,index]
  ncdf_varput,fid,'shortwave_upwelling_flux_w_m02',shortwave_upwelling_fluxes_w_m02,offset=[0,index]
  ncdf_varput,fid,'shortwave_downwelling_flux_w_m02',shortwave_downwelling_fluxes_w_m02,offset=[0,index]
  ncdf_varput,fid,'shortwave_surface_downwelling_flux_w_m02',shortwave_sfc_downwelling_fluxes_w_m02,offset=index
  ncdf_varput,fid,'shortwave_toa_upwelling_flux_w_m02',shortwave_toa_upwelling_fluxes_w_m02,offset=index
  ncdf_varput,fid,'longwave_heating_rate_k_day01',longwave_heating_rates_k_day01,offset=[0,index]
  ncdf_varput,fid,'longwave_upwelling_flux_w_m02',longwave_upwelling_fluxes_w_m02,offset=[0,index]
  ncdf_varput,fid,'longwave_downwelling_flux_w_m02',longwave_downwelling_fluxes_w_m02,offset=[0,index]
  ncdf_varput,fid,'longwave_surface_downwelling_flux_w_m02',longwave_sfc_downwelling_fluxes_w_m02,offset=index
  ncdf_varput,fid,'longwave_toa_upwelling_flux_w_m02',longwave_toa_upwelling_fluxes_w_m02,offset=index
  ncdf_close,fid
...
pro runit, year
  ...
  do_create_output = 1
  outname = string(format='(A,I0,A)','output_file_lwsw.',year,'.cdf')
  ...
  for i=0L,count-1 do begin ; { 
    ...
    rrtm_lw_command = '/home/user/vip/src/rrtm_lw_v3.3/rrtm_v3.3_linux_gfortran'
    ...
    for j=0,n_elements(sites)-1 do begin ; {
      ...
      for k=0,n_elements(hours)-1 do begin ; {
		...
		; Quick QC check
	      ...
	      if(outlw.success eq 0) then begin
	        print,'Warning: RRTM_LW not run properly.  Time to investigate...'
	        continue
	      endif
          ...
          if(npts eq 0) then begin
	        ...
	        output_sw_heating_rates_k_day01 = transpose(outsw.hr)	
	        output_sw_upwelling_fluxes_w_m02 = transpose(outsw.fluxu)	
	        output_sw_downwelling_fluxes_w_m02 = transpose(outsw.fluxdtot)	
	        output_sw_toa_upwelling_fluxes_w_m02 = outsw.osr	
	        output_sw_sfc_downwelling_fluxes_w_m02 = outsw.ssr
	        output_lw_heating_rates_k_day01 = transpose(outlw.hr)
	        output_lw_upwelling_fluxes_w_m02 = transpose(outlw.fluxu)
	        output_lw_downwelling_fluxes_w_m02 = transpose(outlw.fluxd)
	        output_lw_toa_upwelling_fluxes_w_m02 = outlw.olr
	        output_lw_sfc_downwelling_fluxes_w_m02 = outlw.slr
	      endif else begin
	        ...
	        output_sw_heating_rates_k_day01 = [output_sw_heating_rates_k_day01,transpose(outsw.hr)]	
	        output_sw_upwelling_fluxes_w_m02 = [output_sw_upwelling_fluxes_w_m02,transpose(outsw.fluxu)]	
	        output_sw_downwelling_fluxes_w_m02 = [output_sw_downwelling_fluxes_w_m02,transpose(outsw.fluxdtot)]	
	        output_sw_toa_upwelling_fluxes_w_m02 = [output_sw_toa_upwelling_fluxes_w_m02,outsw.osr]	
	        output_sw_sfc_downwelling_fluxes_w_m02 = [output_sw_sfc_downwelling_fluxes_w_m02,outsw.ssr]
	        output_lw_heating_rates_k_day01 = [output_lw_heating_rates_k_day01,transpose(outlw.hr)]
	        output_lw_upwelling_fluxes_w_m02 = [output_lw_upwelling_fluxes_w_m02,transpose(outlw.fluxu)]
	        output_lw_downwelling_fluxes_w_m02 = [output_lw_downwelling_fluxes_w_m02,transpose(outlw.fluxd)]
	        output_lw_toa_upwelling_fluxes_w_m02 = [output_lw_toa_upwelling_fluxes_w_m02,outlw.olr]
	        output_lw_sfc_downwelling_fluxes_w_m02 = [output_lw_sfc_downwelling_fluxes_w_m02,outlw.slr]
	      endelse
	      npts = n_elements(output_latitudes_deg_n)
	    endif
      endfor ; } loop over k
      ...
      if(npts gt 0) then begin
        ...
        output_sw_heating_rates_k_day01 = transpose(output_sw_heating_rates_k_day01)
        output_sw_upwelling_fluxes_w_m02 = transpose(output_sw_upwelling_fluxes_w_m02)
        output_sw_downwelling_fluxes_w_m02 = transpose(output_sw_downwelling_fluxes_w_m02)
        output_lw_heating_rates_k_day01 = transpose(output_lw_heating_rates_k_day01)
        output_lw_upwelling_fluxes_w_m02 = transpose(output_lw_upwelling_fluxes_w_m02)
        output_lw_downwelling_fluxes_w_m02 = transpose(output_lw_downwelling_fluxes_w_m02)
        ...
        index = append_to_output_file(outname, index, output_times_unix_sec, output_julian_days, output_zenith_angles_deg, $
          output_standard_atmo_enums, output_latitudes_deg_n, output_longitudes_deg_e, output_surface_albedos, output_surface_temps_kelvins, output_surface_emissivities, $
	      ...
	      output_sw_heating_rates_k_day01, output_sw_upwelling_fluxes_w_m02, output_sw_downwelling_fluxes_w_m02, output_sw_toa_upwelling_fluxes_w_m02, output_sw_sfc_downwelling_fluxes_w_m02, $
	      output_lw_heating_rates_k_day01, output_lw_upwelling_fluxes_w_m02, output_lw_downwelling_fluxes_w_m02, output_lw_toa_upwelling_fluxes_w_m02, output_lw_sfc_downwelling_fluxes_w_m02)
      endif
    endfor  ; } loop over j
  endfor  ; } loop over i
