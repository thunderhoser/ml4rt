...
# Master script for running the full RRTM (longwave and shortwave).
...
TOP_DATA_DIR_NAME_ACTUAL="/scratch1/RDARCH/rda-ghpcs/Ryan.Lagerquist/ml4rt_project/gfs_data/more_uniform_aod_and_sza/processed/orig_heights"
...
rm -v "${data_dir_name_actual}/output_file_lwsw.${year_string}.cdf"
mkdir -p $data_dir_name_actual
mv $actual_gfs_file_name "${data_dir_name_actual}/"

gdl_script_file_name="${data_dir_name_actual}/run_rrtm_lwsw_${date_string}.gdl"
...
echo ".run /home/user/data/run_rrtm_lwsw_orig_heights.pro" >> $gdl_script_file_name
