...
THESE_WAVENUMBERS_CM01 = numpy.array([
    2925, 3625, 4325, 4900, 5650, 6925, 7875, 10450, 14425, 19325, 25825, 33500,
    44000, 1710
], dtype=float)
SHORTWAVE_WAVELENGTHS_METRES = 0.01 * (THESE_WAVENUMBERS_CM01 ** -1)
SHORTWAVE_WAVELENGTHS_METRES = numpy.concatenate([
    SHORTWAVE_WAVELENGTHS_METRES,
    numpy.array([example_utils.DUMMY_BROADBAND_WAVELENGTH_METRES])
])

THESE_WAVENUMBERS_CM01 = numpy.array([
    180, 425, 565, 665, 760, 900, 1030, 1130, 1285, 1435, 1640, 1940, 2165,
    2315, 2490, 2925
], dtype=float)
LONGWAVE_WAVELENGTHS_METRES = 0.01 * (THESE_WAVENUMBERS_CM01 ** -1)
LONGWAVE_WAVELENGTHS_METRES = numpy.concatenate([
    LONGWAVE_WAVELENGTHS_METRES,
    numpy.array([example_utils.DUMMY_BROADBAND_WAVELENGTH_METRES])
])

VALID_TIMES_KEY = 'valid_time_unix_sec'
HEIGHTS_KEY = 'height_m_agl'
STANDARD_ATMO_FLAGS_KEY = 'standard_atmosphere_enum'
...
def read_file(netcdf_file_name, dummy_heights_m_agl, look_for_shortwave,
    ...
    :param dummy_heights_m_agl: length-H numpy array of dummy heights (metres
        above ground level).
    :param look_for_shortwave: Boolean flag.  If True (False), will look for
        shortwave (longwave) results.
    ...
    example_dict['heights_m_agl']: length-H numpy array of heights (metres above
        ground level).
    example_dict['target_wavelengths_metres']: length-W numpy array of target
        wavelengths.
    ...
    if look_for_shortwave:
        scalar_target_names = SHORTWAVE_SCALAR_TARGET_NAMES
        vector_target_names = SHORTWAVE_VECTOR_TARGET_NAMES
        required_predictor_names = REQUIRED_SHORTWAVE_PREDICTOR_NAMES
        target_wavelengths_metres = SHORTWAVE_WAVELENGTHS_METRES
    else:
        scalar_target_names = LONGWAVE_SCALAR_TARGET_NAMES
        vector_target_names = LONGWAVE_VECTOR_TARGET_NAMES
        required_predictor_names = REQUIRED_LONGWAVE_PREDICTOR_NAMES
        target_wavelengths_metres = LONGWAVE_WAVELENGTHS_METRES

    scalar_target_names_orig = [
        TARGET_NAME_TO_ORIG[n] for n in scalar_target_names
    ]
    vector_target_names_orig = [
        TARGET_NAME_TO_ORIG[n] for n in vector_target_names
    ]
    ...
    # Read file and do more housekeeping.
    ...
    for this_name in SCALAR_PREDICTOR_NAMES:
        ...
        if (
                PREDICTOR_NAME_TO_ORIG[this_name] not in
    ...
    error_checking.assert_is_greater_numpy_array(dummy_heights_m_agl, 0)
    ...
    example_dict = {
        example_utils.HEIGHTS_KEY: dummy_heights_m_agl,
    ...
    # Finally, read the predictor and target values.
    num_examples = len(example_dict[example_utils.VALID_TIMES_KEY])
    ...
    scalar_target_matrix = numpy.full(
        (num_examples, num_target_wavelengths, num_scalar_targets), numpy.nan
    )
    vector_target_matrix = numpy.full(
        (num_examples, num_heights, num_target_wavelengths, num_vector_targets),
        numpy.nan
    )
    ...
    for k in range(num_vector_targets):
        this_data_matrix = numpy.array(
            dataset_object.variables[vector_target_names_orig[k]][:],
            dtype=float
        )
        vector_target_matrix[..., k] = numpy.swapaxes(this_data_matrix, 1, 2)
