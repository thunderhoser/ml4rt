...
def _run(input_prediction_file_name, for_shortwave, confidence_level,
    ...
    num_examples = len(targets_w_m02)
    pit_values = numpy.full(num_examples, numpy.nan)

    for i in range(num_examples):
        if targets_w_m02[i] > numpy.max(prediction_matrix_w_m02[i, :]):
            pit_values[i] = (
                1. + targets_w_m02[i] - numpy.max(prediction_matrix_w_m02[i, :])
            )
            continue

        if targets_w_m02[i] < numpy.min(prediction_matrix_w_m02[i, :]):
            pit_values[i] = (
                targets_w_m02[i] - numpy.min(prediction_matrix_w_m02[i, :])
            )
            continue

        pit_values[i] = 0.01 * percentileofscore(
            a=prediction_matrix_w_m02[i, :],
            score=targets_w_m02[i], kind='mean'
        )

    min_predictions_w_m02 = numpy.percentile(
        prediction_matrix_w_m02, 50 * (1. - confidence_level), axis=-1
    )
    max_predictions_w_m02 = numpy.percentile(
        prediction_matrix_w_m02, 50 * (1. + confidence_level), axis=-1
    )
    total_uncertainties_w_m02 = max_predictions_w_m02 - min_predictions_w_m02

    print(SEPARATOR_STRING)
    high_bias_indices, low_bias_indices, low_abs_error_indices = (
        misc_utils.find_best_and_worst_predictions(
            bias_matrix=0.5 - numpy.expand_dims(pit_values, axis=1),
            absolute_error_matrix=numpy.absolute(
                numpy.expand_dims(pit_values, axis=1) - 0.5
            ),
    ...
    for i in range(num_examples_per_set):
        print('{0:d}th-highest uncertainty = {1:f}'.format(
            i + 1,
            total_uncertainties_w_m02[high_uncertainty_indices[i]]
        ))
    ...
    for i in range(num_examples_per_set):
        print('{0:d}th-lowest uncertainty = {1:f}'.format(
            i + 1,
            total_uncertainties_w_m02[low_uncertainty_indices[i]]
        ))
