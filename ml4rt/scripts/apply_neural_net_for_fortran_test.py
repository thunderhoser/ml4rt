"""Applies trained neural net for Fortran unit test by Dustin."""
...
EXAMPLE_DIM = 'example'
HEIGHT_DIM = 'height_sigma_level'
WAVELENGTH_DIM = 'wavelength_microns'
PREDICTOR_DIM = 'predictor_variable'
FLUX_DIM = 'flux_variable'
...
def _write_predictors_and_predictions(
        netcdf_file_name, predictor_matrix,
        vector_prediction_matrix, scalar_prediction_matrix,
        predictor_names, flux_variable_names, wavelengths_microns):
    ...
    :param flux_variable_names: 1-D list with names of flux variables.
    :param wavelengths_microns: 1-D numpy array of wavelengths.
    """

    num_examples = predictor_matrix.shape[0]
    num_heights = predictor_matrix.shape[1]
    coord_dict = {
        EXAMPLE_DIM: numpy.linspace(
            0, num_examples - 1, num=num_examples, dtype=int
        ),
        HEIGHT_DIM: numpy.linspace(
            0, num_heights - 1, num=num_heights, dtype=int
        ),
        WAVELENGTH_DIM: wavelengths_microns,
        PREDICTOR_DIM: predictor_names,
        FLUX_DIM: flux_variable_names
    }

    main_data_dict = {
        PREDICTOR_MATRIX_KEY: (
            (EXAMPLE_DIM, HEIGHT_DIM, PREDICTOR_DIM),
            predictor_matrix
        ),
        VECTOR_PREDICTION_MATRIX_KEY: (
            (EXAMPLE_DIM, HEIGHT_DIM, WAVELENGTH_DIM),
            vector_prediction_matrix
        ),
        SCALAR_PREDICTION_MATRIX_KEY: (
            (EXAMPLE_DIM, WAVELENGTH_DIM, FLUX_DIM),
            scalar_prediction_matrix
        )
    }

    output_table_xarray = xarray.Dataset(
        data_vars=main_data_dict, coords=coord_dict
    )

    file_system_utils.mkdir_recursive_if_necessary(file_name=netcdf_file_name)
    output_table_xarray.to_netcdf(
...
def _run(model_file_name, iso_reg_model_file_name, iso_reg_layer_type_string,
    ...
    sel_indices = numpy.linspace(
        0, len(example_id_strings) - 1, num=len(example_id_strings), dtype=int
    )
    sel_indices = numpy.random.choice(
        sel_indices, size=100, replace=False
    )

    example_id_strings = [example_id_strings[k] for k in sel_indices]
    predictor_matrix = predictor_matrix[sel_indices, ...]
    for this_key in target_dict:
        target_dict[this_key] = target_dict[this_key][sel_indices, ...]
    ...
    all_predictor_names = (
        goptd[neural_net.VECTOR_PREDICTOR_NAMES_KEY] +
        goptd[neural_net.SCALAR_PREDICTOR_NAMES_KEY]
    )
    ...
    _write_predictors_and_predictions(
        ...
        vector_prediction_matrix=vector_prediction_matrix[..., 0, 0],
        scalar_prediction_matrix=scalar_prediction_matrix[..., 0],
        predictor_names=all_predictor_names,
        flux_variable_names=goptd[neural_net.SCALAR_TARGET_NAMES_KEY],
        wavelengths_microns=1e6 * goptd[neural_net.TARGET_WAVELENGTHS_KEY]
    )
