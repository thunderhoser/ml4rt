...
def _run(net_type_string, training_dir_name, validation_dir_name,
    ...
    bayesian_neural_net.check_net_type(net_type_string)
    ...
    training_option_dict = {
        bayesian_neural_net.EXAMPLE_DIRECTORY_KEY: training_dir_name,
        bayesian_neural_net.BATCH_SIZE_KEY: num_examples_per_batch,
        bayesian_neural_net.SCALAR_PREDICTOR_NAMES_KEY: scalar_predictor_names,
        bayesian_neural_net.VECTOR_PREDICTOR_NAMES_KEY: vector_predictor_names,
        bayesian_neural_net.SCALAR_TARGET_NAMES_KEY: scalar_target_names,
        bayesian_neural_net.VECTOR_TARGET_NAMES_KEY: vector_target_names,
        bayesian_neural_net.HEIGHTS_KEY: heights_m_agl,
        bayesian_neural_net.MULTIPLY_PREDS_BY_THICKNESS_KEY:
            multiply_preds_by_layer_thickness,
        bayesian_neural_net.NORMALIZATION_FILE_KEY: normalization_file_name,
        bayesian_neural_net.UNIFORMIZE_FLAG_KEY: uniformize,
        bayesian_neural_net.PREDICTOR_NORM_TYPE_KEY: predictor_norm_type_string,
        bayesian_neural_net.PREDICTOR_MIN_NORM_VALUE_KEY: predictor_min_norm_value,
        bayesian_neural_net.PREDICTOR_MAX_NORM_VALUE_KEY: predictor_max_norm_value,
        bayesian_neural_net.VECTOR_TARGET_NORM_TYPE_KEY: vector_target_norm_type_string,
        bayesian_neural_net.VECTOR_TARGET_MIN_VALUE_KEY: vector_target_min_norm_value,
        bayesian_neural_net.VECTOR_TARGET_MAX_VALUE_KEY: vector_target_max_norm_value,
        bayesian_neural_net.SCALAR_TARGET_NORM_TYPE_KEY: scalar_target_norm_type_string,
        bayesian_neural_net.SCALAR_TARGET_MIN_VALUE_KEY: scalar_target_min_norm_value,
        bayesian_neural_net.SCALAR_TARGET_MAX_VALUE_KEY: scalar_target_max_norm_value,
        bayesian_neural_net.FIRST_TIME_KEY: first_training_time_unix_sec,
        bayesian_neural_net.LAST_TIME_KEY: last_training_time_unix_sec,
        bayesian_neural_net.JOINED_OUTPUT_LAYER_KEY: joined_output_layer,
        bayesian_neural_net.NUM_DEEP_SUPER_LAYERS_KEY: num_deep_supervision_layers
    }

    validation_option_dict = {
        bayesian_neural_net.EXAMPLE_DIRECTORY_KEY: validation_dir_name,
        bayesian_neural_net.BATCH_SIZE_KEY: num_examples_per_batch,
        bayesian_neural_net.FIRST_TIME_KEY: first_validn_time_unix_sec,
        bayesian_neural_net.LAST_TIME_KEY: last_validn_time_unix_sec
    }

    print('Reading untrained model from: "{0:s}"...'.format(
        input_model_file_name
    ))
    model_object = bayesian_neural_net.read_model(input_model_file_name)

    # input_metafile_name = neural_net.find_metafile(
    #     model_dir_name=os.path.split(input_model_file_name)[0]
    # )
    input_metafile_name = bayesian_neural_net.find_metafile(
        model_dir_name='/'.join(input_model_file_name.split('/')[:-1])
    )

    print('Reading loss function(s) from: "{0:s}"...'.format(
        input_metafile_name
    ))
    loss_function_or_dict = bayesian_neural_net.read_metafile(
        input_metafile_name
    )[bayesian_neural_net.LOSS_FUNCTION_OR_DICT_KEY]

    print(SEPARATOR_STRING)

    if use_generator_for_training:
        bayesian_neural_net.train_model_with_generator(
        ...
        bayesian_neural_net.train_model_sans_generator(
            model_object=model_object, output_dir_name=output_model_dir_name,
