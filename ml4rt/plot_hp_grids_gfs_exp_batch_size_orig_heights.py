...
USE_GENERATOR_FLAGS = numpy.array([0, 1], dtype=bool)

EXAMPLES_PER_BATCH_COUNTS = numpy.round(
    numpy.logspace(6, 10, num=9, base=2)
).astype(int)

BATCHES_PER_EPOCH_COUNTS = numpy.array(
    [100, 250, 500, 750, 1000, 1500, 2000, 2500, 3000, 4000, 5000], dtype=int
)
...
def _print_ranking_one_score(score_matrix, score_name):
    ...
    G = number of Boolean flags indicating whether or not generator was used
    E = number of examples-per-batch counts
    B = number of batches-per-epoch counts

    :param score_matrix: G-by-E-by-B numpy array of scores.
    ...
    for m in range(len(i_sort_indices)):
        ...
        print((
            '{0:d}th-lowest {1:s} = {2:.4g} ... generator = {3:s} ... '
            'examples per batch = {4:d} ... batches per epoch = {5:d}'
        ).format(
            m + 1, score_name, score_matrix[i, j, k],
            'YES' if USE_GENERATOR_FLAGS[i] else 'NO',
            EXAMPLES_PER_BATCH_COUNTS[j],
            BATCHES_PER_EPOCH_COUNTS[k]
        ))
...
def _print_ranking_all_scores(
    ...
    :param prmse_matrix_k_day01: G-by-E-by-B numpy array of profile RMSE.
    :param dwmse_matrix_k3_day03: G-by-E-by-B numpy array of dual-weighted MSE.
    :param down_flux_rmse_matrix_w_m02: G-by-E-by-B numpy array of RMSE for
        surface downwelling flux.
    :param up_flux_rmse_matrix_w_m02: G-by-E-by-B numpy array of RMSE for TOA
        upwelling flux.
    :param net_flux_rmse_matrix_w_m02: G-by-E-by-B numpy array of RMSE for net
        flux.
    """
    ...
    for m in range(len(i_sort_indices)):
        ...
        print((
            '{0:d}th-lowest PRMSE = {1:.4g} K day^-1 ... generator = {2:s} ...'
            'examples per batch = {3:d} ... batches per epoch = {4:d} ... '
            'DWMSE rank = {5:.1f} ... down-flux-RMSE rank = {6:.1f} ... '
            'up-flux-RMSE rank = {7:.1f} ... net-flux-RMSE rank = {8:.1f}'
        ).format(
            m + 1, prmse_matrix_k_day01[i, j, k],
            'YES' if USE_GENERATOR_FLAGS[i] else 'NO',
            EXAMPLES_PER_BATCH_COUNTS[j], BATCHES_PER_EPOCH_COUNTS[k],
...
def _run(experiment_dir_name, isotonic_flag):
    ...
    num_generator_flags = len(USE_GENERATOR_FLAGS)
    ...
    if os.path.isfile(score_file_name):
        ...
        dimensions = (num_generator_flags, num_example_counts, num_batch_counts)
        ...
        for i in range(num_generator_flags):
            for j in range(num_example_counts):
                for k in range(num_batch_counts):
                    this_model_dir_name = (
                        '{0:s}/use-generator={1:d}_'
                        'num-examples-per-batch={2:03d}_'
                        'num-batches-per-epoch={3:04d}'
                    ).format(
                        experiment_dir_name, int(USE_GENERATOR_FLAGS[i]),
                        EXAMPLES_PER_BATCH_COUNTS[j],
                        BATCHES_PER_EPOCH_COUNTS[k]
                    )
    ...
    for i in range(num_generator_flags):
        ...
        figure_file_name = (
            '{0:s}/{1:s}prmse_grid_use-generator={2:d}.jpg'
        ).format(
            experiment_dir_name,
            'isotonic_regression/' if isotonic_flag else '',
            int(USE_GENERATOR_FLAGS[i])
        )
        ...
        figure_file_name = (
            '{0:s}/{1:s}dwmse_grid_use-generator={2:d}.jpg'
        ).format(
            experiment_dir_name,
            'isotonic_regression/' if isotonic_flag else '',
            int(USE_GENERATOR_FLAGS[i])
        ...
        axes_object.set_xlabel(x_axis_label)
        axes_object.set_ylabel(y_axis_label)
        axes_object.set_title(r'RMSE for upwelling flux (W m$^{-2}$)')

        figure_file_name = (
            '{0:s}/{1:s}up-flux-rmse_grid_use-generator={2:d}.jpg'
        ).format(
            experiment_dir_name,
            'isotonic_regression/' if isotonic_flag else '',
            int(USE_GENERATOR_FLAGS[i])
        ...
        figure_object, axes_object = _plot_scores_2d(
            ...
            max_colour_value=
            numpy.nanpercentile(down_flux_rmse_matrix_w_m02, 100),
            x_tick_labels=x_tick_labels, y_tick_labels=y_tick_labels
        )
        ...
        axes_object.set_xlabel(x_axis_label)
        axes_object.set_ylabel(y_axis_label)
        axes_object.set_title(r'RMSE for downwelling flux (W m$^{-2}$)')

        figure_file_name = (
            '{0:s}/{1:s}down-flux-rmse_grid_use-generator={2:d}.jpg'
        ).format(
            experiment_dir_name,
            'isotonic_regression/' if isotonic_flag else '',
            int(USE_GENERATOR_FLAGS[i])
        ...
        figure_file_name = (
            '{0:s}/{1:s}net-flux-rmse_grid_use-generator={2:d}.jpg'
        ).format(
            experiment_dir_name,
            'isotonic_regression/' if isotonic_flag else '',
            int(USE_GENERATOR_FLAGS[i])
