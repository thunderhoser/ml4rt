"""Makes U-net++ templates for GFS experiment with grid-aware models."""
...
OUTPUT_DIR_NAME = (
    '{0:s}/ml4rt_models/gfs_experiment_grid_aware/templates'
).format(HOME_DIR_NAME)

USE_HEIGHT_FLAGS = numpy.array([0, 1], dtype=bool)
DENSE_LAYER_COUNTS = numpy.array([4], dtype=int)
DENSE_LAYER_DROPOUT_RATES = numpy.array([0.])
L2_WEIGHTS = numpy.array([1e-7])
...
def _run():
    """Makes U-net++ template for GFS experiment with batch size on orig hgts.
    ...
    num_use_height_flags = len(USE_HEIGHT_FLAGS)
            ...
            for j in range(num_dropout_rates):
                ...
                for k in range(num_l2_weights):
                    ...
                    this_option_dict[
                        ...
                        [127, 21 + int(USE_HEIGHT_FLAGS[m])], dtype=int
                    )
                    ...
                    this_model_file_name = (
                        '{0:s}/model_use-height={1:d}.h5'
                    ).format(OUTPUT_DIR_NAME, int(USE_HEIGHT_FLAGS[m]))
