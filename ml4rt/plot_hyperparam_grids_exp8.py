...
SUPER_RES_FLAG_ARG_NAME = 'super_resolution_flag'
...
SUPER_RES_FLAG_HELP_STRING = (
    'Boolean flag.  If 1 (0), will plot results with(out) super-resolution.'
)
...
INPUT_ARG_PARSER.add_argument(
    '--' + SUPER_RES_FLAG_ARG_NAME, type=int, required=False, default=0,
    help=SUPER_RES_FLAG_HELP_STRING
)
...
def _read_scores_one_model(model_dir_name, isotonic_flag,
    ...
    :param super_resolution_flag: Same.
    ...
    evaluation_file_name = '{0:s}/{1:s}{2:s}validation/evaluation.nc'.format(
        model_file_name[:-3],
        '192heights/' if super_resolution_flag else '',
...
def _run(experiment_dir_name, isotonic_flag, super_resolution_flag):
    """Plots scores on hyperparameter grid for Experiment 8.
    ...
    score_file_name = '{0:s}/{1:s}{2:s}scores_on_hyperparam_grid.p'.format(
        experiment_dir_name,
        'super_resolution/' if super_resolution_flag else '',
        'isotonic_regression/' if isotonic_flag else ''
    )
            ...
            for j in range(num_dropout_rates):
                for k in range(num_l2_weights):
                    ...
                    (
                        ...
                        super_resolution_flag=super_resolution_flag
                    )
...
if __name__ == '__main__':
    ...
    _run(
        ...
        super_resolution_flag=bool(getattr(
            INPUT_ARG_OBJECT, SUPER_RES_FLAG_ARG_NAME
        ))
    )
