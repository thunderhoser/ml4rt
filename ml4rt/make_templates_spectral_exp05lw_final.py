"""Makes final templates for Spectral Experiment 5-LW.

Same as the rest of Spectral Experiment 5-LW, but with residual blocks.
"""
...
OUTPUT_DIR_NAME = (
    '{0:s}/ml4rt_models/spectral_experiment05lw_final/templates'
).format(HOME_DIR_NAME)

ENSEMBLE_SIZE = 1
NUM_WAVELENGTHS = 16

# Create most of U-net++ architecture.
THIS_MODEL_DEPTH = 3
NUM_CONV_LAYERS_PER_BLOCK = 1
THIS_SPECTRAL_COMPLEXITY = 128

ALL_CHANNEL_COUNTS = (
    THIS_SPECTRAL_COMPLEXITY *
    numpy.logspace(0, THIS_MODEL_DEPTH, num=THIS_MODEL_DEPTH + 1, base=2.)
)
...
DEFAULT_PLUSPLUS_OPTION_DICT = {
    u_net_pp_arch.INPUT_DIMENSIONS_KEY: numpy.array([127, 24], dtype=int),
    u_net_pp_arch.NUM_LEVELS_KEY: THIS_MODEL_DEPTH,
    u_net_pp_arch.CONV_LAYER_COUNTS_KEY: numpy.full(
        THIS_MODEL_DEPTH + 1, NUM_CONV_LAYERS_PER_BLOCK, dtype=int
    ),
    u_net_pp_arch.CHANNEL_COUNTS_KEY: ALL_CHANNEL_COUNTS,
    u_net_pp_arch.ENCODER_DROPOUT_RATES_KEY: numpy.full(THIS_MODEL_DEPTH + 1, 0.),
    u_net_pp_arch.ENCODER_MC_DROPOUT_FLAGS_KEY: numpy.full(
        THIS_MODEL_DEPTH + 1, False, dtype=bool
    ),
    u_net_pp_arch.UPCONV_DROPOUT_RATES_KEY: numpy.full(THIS_MODEL_DEPTH, 0.),
    u_net_pp_arch.UPCONV_MC_DROPOUT_FLAGS_KEY: numpy.full(
        THIS_MODEL_DEPTH, False, dtype=bool
    ),
    u_net_pp_arch.SKIP_DROPOUT_RATES_KEY: numpy.full(THIS_MODEL_DEPTH, 0.),
    ...
    u_net_pp_arch.CONV_OUTPUT_ACTIV_FUNC_KEY: None,
    u_net_pp_arch.CONV_OUTPUT_ACTIV_FUNC_ALPHA_KEY: 0.,
    u_net_pp_arch.DENSE_OUTPUT_ACTIV_FUNC_KEY: None,
...
# Create most of U-net3+ architecture.
THIS_MODEL_DEPTH = 5
THIS_SPECTRAL_COMPLEXITY = 64
...
DEFAULT_3PLUS_OPTION_DICT = {
    u_net_ppp_arch.INPUT_DIMENSIONS_KEY: numpy.array([127, 24], dtype=int),
    u_net_ppp_arch.NUM_LEVELS_KEY: THIS_MODEL_DEPTH,
    u_net_ppp_arch.CONV_LAYER_COUNTS_KEY: numpy.full(
        THIS_MODEL_DEPTH + 1, NUM_CONV_LAYERS_PER_BLOCK, dtype=int
    ),
    u_net_ppp_arch.CHANNEL_COUNTS_KEY: ALL_CHANNEL_COUNTS,
    u_net_ppp_arch.ENCODER_DROPOUT_RATES_KEY: numpy.full(THIS_MODEL_DEPTH + 1, 0.),
    u_net_ppp_arch.ENCODER_MC_DROPOUT_FLAGS_KEY: numpy.full(
        THIS_MODEL_DEPTH + 1, False, dtype=bool
    ),
    u_net_ppp_arch.UPCONV_DROPOUT_RATES_KEY: numpy.full(THIS_MODEL_DEPTH, 0.),
    u_net_ppp_arch.UPCONV_MC_DROPOUT_FLAGS_KEY: numpy.full(
        THIS_MODEL_DEPTH, False, dtype=bool
    ),
    u_net_ppp_arch.SKIP_DROPOUT_RATES_KEY: numpy.full(THIS_MODEL_DEPTH, 0.),
    ...
    u_net_ppp_arch.INCLUDE_MASK_KEY: True
}
...
MIN_DUAL_WEIGHTS = numpy.array([0.75, 0.75])
BROADBAND_WEIGHTS = numpy.array([0.01, 0.01])
ARCHITECTURE_TYPE_STRINGS = ['plusplus', 'plusplusplus']


def _run():
    """Makes templates for Spectral Experiment 5-LW.

    This is effectively the main method.
    """

    for i in range(len(MIN_DUAL_WEIGHTS)):
        if ARCHITECTURE_TYPE_STRINGS[i] == 'plusplus':
            this_option_dict = copy.deepcopy(DEFAULT_PLUSPLUS_OPTION_DICT)
        else:
            this_option_dict = copy.deepcopy(DEFAULT_3PLUS_OPTION_DICT)
        ...
        this_model_file_name = (
            ...
            'architecture-type={3:s}/model.keras'
        ).format(
            OUTPUT_DIR_NAME,
            MIN_DUAL_WEIGHTS[i],
            BROADBAND_WEIGHTS[i],
            ARCHITECTURE_TYPE_STRINGS[i]
        )
        ...
        neural_net._write_metafile(
            ...
            u_net_3plus_architecture_dict=(
                this_option_dict
                if ARCHITECTURE_TYPE_STRINGS[i] == 'plusplusplus'
                else None
            ),
