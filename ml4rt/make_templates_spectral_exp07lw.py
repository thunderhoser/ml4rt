"""Makes templates for Spectral Experiment 7 with longwave.
...
import u_net_ppp_architecture as u_net_ppp_arch
...
OUTPUT_DIR_NAME = (
    '{0:s}/ml4rt_models/spectral_experiment07_longwave/templates'
...
VECTOR_PREDICTOR_NAMES = [
    ...
    'n2o_concentration_ppmv', 'height_m_agl', 'height_thickness_metres',
    'pressure_thickness_pascals'
]
SCALAR_PREDICTOR_NAMES = [
    'zenith_angle_radians', 'surface_temperature_kelvins', 'surface_emissivity'
]
HEIGHTS_M_AGL = neural_net.HEIGHTS_FOR_PETER_M_AGL

VECTOR_TARGET_NAME = 'longwave_heating_rate_k_day01'
SCALAR_TARGET_NAMES = [
    'longwave_surface_down_flux_w_m02', 'longwave_toa_up_flux_w_m02'
]
WAVELENGTHS_METRES = numpy.array([
    0.000055555555556, 0.000023529411765, 0.000017699115044, 0.000015037593985,
    0.000013157894737, 0.000011111111111, 0.000009708737864, 0.000008849557522,
    0.000007782101167, 0.000006968641115, 0.000006097560976, 0.000005154639175,
    0.000004618937644, 0.000004319654428, 0.000004016064257, 0.000003418803419
])
...
DENSE_LAYER_NEURON_COUNTS = architecture_utils.get_dense_layer_dimensions(
    num_input_units=int(numpy.round(3 * ALL_CHANNEL_COUNTS[-1])),
...
DEFAULT_OPTION_DICT = {
    u_net_ppp_arch.INPUT_DIMENSIONS_KEY: numpy.array([127, 24], dtype=int),
    u_net_ppp_arch.NUM_LEVELS_KEY: MODEL_DEPTH,
    u_net_ppp_arch.CONV_LAYER_COUNTS_KEY: numpy.full(
        MODEL_DEPTH + 1, NUM_CONV_LAYERS_PER_BLOCK, dtype=int
    ),
    u_net_ppp_arch.CHANNEL_COUNTS_KEY: ALL_CHANNEL_COUNTS,
    u_net_ppp_arch.ENCODER_DROPOUT_RATES_KEY: numpy.full(MODEL_DEPTH + 1, 0.),
    u_net_ppp_arch.ENCODER_MC_DROPOUT_FLAGS_KEY: numpy.full(
        MODEL_DEPTH + 1, False, dtype=bool
    ),
    u_net_ppp_arch.UPCONV_DROPOUT_RATES_KEY: numpy.full(MODEL_DEPTH, 0.),
    u_net_ppp_arch.UPCONV_MC_DROPOUT_FLAGS_KEY: numpy.full(
        MODEL_DEPTH, False, dtype=bool
    ),
    u_net_ppp_arch.SKIP_DROPOUT_RATES_KEY: numpy.full(MODEL_DEPTH, 0.),
    u_net_ppp_arch.SKIP_MC_DROPOUT_FLAGS_KEY: numpy.full(
        MODEL_DEPTH, False, dtype=bool
    ),
    u_net_ppp_arch.INCLUDE_PENULTIMATE_KEY: False,
    u_net_ppp_arch.DENSE_LAYER_NEURON_NUMS_KEY: DENSE_LAYER_NEURON_COUNTS,
    u_net_ppp_arch.DENSE_LAYER_DROPOUT_RATES_KEY: numpy.full(4, 0.),
    u_net_ppp_arch.DENSE_LAYER_MC_DROPOUT_FLAGS_KEY: numpy.full(
        4, False, dtype=bool
    ),
    u_net_ppp_arch.INNER_ACTIV_FUNCTION_KEY:
        architecture_utils.RELU_FUNCTION_STRING,
    u_net_ppp_arch.INNER_ACTIV_FUNCTION_ALPHA_KEY: 0.2,
    u_net_ppp_arch.CONV_OUTPUT_ACTIV_FUNC_KEY: None,
    u_net_ppp_arch.CONV_OUTPUT_ACTIV_FUNC_ALPHA_KEY: 0.,
    u_net_ppp_arch.DENSE_OUTPUT_ACTIV_FUNC_KEY: None,
    u_net_ppp_arch.DENSE_OUTPUT_ACTIV_FUNC_ALPHA_KEY: 0.,
    u_net_ppp_arch.L1_WEIGHT_KEY: 0.,
    u_net_ppp_arch.L2_WEIGHT_KEY: 1e-7,
    u_net_ppp_arch.USE_BATCH_NORM_KEY: True,
    # u_net_ppp_arch.USE_RESIDUAL_BLOCKS_KEY: True,
    u_net_ppp_arch.USE_CONVNEXT_V1_BLOCKS_KEY: False,
    u_net_ppp_arch.USE_CONVNEXT_V2_BLOCKS_KEY: False,
    u_net_ppp_arch.SIMPLIFY_CONVNEXT_KEY: False,
    u_net_ppp_arch.SIMPLIFY_OUTPUT_LAYER_KEY: False,
    # u_net_ppp_arch.MEAN_VALUE_MATRIX_KEY: None,
    # u_net_ppp_arch.STDEV_MATRIX_KEY: None,
    # u_net_ppp_arch.HEATING_RATE_MASK_KEY: None,
    # u_net_ppp_arch.FLUX_MASK_KEY: None,
    u_net_ppp_arch.NUM_OUTPUT_WAVELENGTHS_KEY: NUM_WAVELENGTHS,
    # u_net_ppp_arch.VECTOR_LOSS_FUNCTION_KEY: VECTOR_LOSS_FUNCTION,
    # u_net_ppp_arch.SCALAR_LOSS_FUNCTION_KEY: SCALAR_LOSS_FUNCTION,
    u_net_ppp_arch.USE_DEEP_SUPERVISION_KEY: False,
    u_net_ppp_arch.ENSEMBLE_SIZE_KEY: ENSEMBLE_SIZE
}
...
def _run():
    ...
    for j in range(len(USE_RESIDUAL_FLAGS)):
        ...
        this_option_dict.update({
            u_net_ppp_arch.VECTOR_LOSS_FUNCTION_KEY: this_vector_loss_function,
            u_net_ppp_arch.SCALAR_LOSS_FUNCTION_KEY: this_scalar_loss_function,
            u_net_ppp_arch.USE_RESIDUAL_BLOCKS_KEY: USE_RESIDUAL_FLAGS[j],
            u_net_ppp_arch.MEAN_VALUE_MATRIX_KEY: mean_value_matrix,
            u_net_ppp_arch.STDEV_MATRIX_KEY: stdev_matrix,
            u_net_ppp_arch.HEATING_RATE_MASK_KEY: heating_rate_mask_matrix,
            u_net_ppp_arch.FLUX_MASK_KEY: flux_mask_matrix
        })

        this_model_object = u_net_ppp_arch.create_model(this_option_dict)
        ...
        this_option_dict[u_net_ppp_arch.VECTOR_LOSS_FUNCTION_KEY] = (
            this_vector_loss_string
        )
        this_option_dict[u_net_ppp_arch.SCALAR_LOSS_FUNCTION_KEY] = (
            this_scalar_loss_string
        )
