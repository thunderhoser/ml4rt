...
Same as Spectral Experiment 5, but for longwave instead of shortwave.
"""
...
BROADBAND_WEIGHTS = numpy.array([0.01, 0.05, 0.10, 0.20])
...
def _run():
    ...
    for i in range(len(MIN_DUAL_WEIGHTS)):
        for j in range(len(BROADBAND_WEIGHTS)):
            ...
            this_model_file_name = (
                '{0:s}/min-dual-weight={1:.2f}_broadband-weight={2:.2f}/'
                'model.keras'
            ).format(
                OUTPUT_DIR_NAME,
                MIN_DUAL_WEIGHTS[i],
