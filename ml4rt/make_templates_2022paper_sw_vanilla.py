"""Makes vanilla U-net templates for shortwave -- not actually used in paper."""
...
OUTPUT_DIR_NAME = (
    '{0:s}/ml4rt_models/2022paper_experiment_sw_vanilla/templates'
...
DEFAULT_OPTION_DICT = {
    ...
    u_net_architecture.DENSE_LAYER_DROPOUT_RATES_KEY:
        numpy.array([0.1, 0.1, 0.1, 0]),
    u_net_architecture.DENSE_LAYER_MC_DROPOUT_FLAGS_KEY:
        numpy.array([1, 1, 1, 0], dtype=bool)
}
        ...
        for j in range(num_conv_layer_counts):
            for k in range(num_channel_counts):
                ...
                this_option_dict.update({
                    u_net_architecture.NUM_LEVELS_KEY: MODEL_DEPTHS[i],
                    u_net_architecture.CONV_LAYER_COUNTS_KEY:
                        numpy.full(
                            MODEL_DEPTHS[i] + 1, CONV_LAYER_COUNTS[j], dtype=int
                        ),
                    u_net_architecture.CHANNEL_COUNTS_KEY:
                        these_channel_counts_all,
                    u_net_architecture.ENCODER_DROPOUT_RATES_KEY:
