"""Plots each evaluation score vs. hyperparams for 2022 longwave experiment."""
...
def _read_scores_one_model(model_dir_name, multilayer_cloud_flag, fog_flag,
    ...
    evaluation_file_name = '{0:s}/{1:s}validation/{2:s}evaluation.nc'.format(
        ...
        'by_cloud_regime/liquid_only/multi_layer_cloud/' if multilayer_cloud_flag
        else 'by_cloud_regime/liquid_only/fog/' if fog_flag
        else ''
    )
...
def _run(experiment_dir_name, isotonic_flag):
    """Plots each evaluation score vs. hyperparams for 2022 LW experiment.
